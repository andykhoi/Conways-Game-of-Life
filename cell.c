...
bool Cell_AreNeighbors(Cell C1, Cell C2) {
    ...
    if (C1.x == C2.x - 1 && C1.y == C2.y) { //right
        return true;
    }
    else if (C1.x == C2.x + 1 && C1.y == C2.y) { //left
	return true;
    }
    else if (C1.x == C2.x && C1.y == C2.y - 1) { //down
	return true;
    }
    else if (C1.x == C2.x && C1.y == C2.y + 1) { //up
	return true;
    }
    else if (C1.x == C2.x - 1 && C1.y == C2.y - 1) { //down right
	return true;
    }
    else if (C1.x == C2.x + 1 && C1.y == C2.y - 1) { //down left
	return true;
    }
    else if (C1.x == C2.x - 1 && C1.y == C2.y + 1) { //up right
	return true;
    }
    else if (C1.x == C2.x + 1 && C1.y == C2.y + 1) { //up left
	return true;
    }
    else {
	return false;
